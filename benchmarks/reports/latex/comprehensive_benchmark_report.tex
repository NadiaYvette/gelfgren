...
\begin{enumerate}
\item \textbf{Implement True Rational BVP Solver}:
  \begin{itemize}
  \item Rational Galerkin discretization
  \item Rational collocation methods
  \item Optimization-based approach using HermiteConstraints interface
  \end{itemize}

\item \textbf{Benchmark Genuine Rational BVP Methods}:
  \begin{itemize}
  \item Compare rational vs polynomial basis functions for discretization
  \item Test on problems where rationals should excel (near-singular solutions)
  \item Measure computational cost vs accuracy trade-offs
  \end{itemize}
