...
Direct approximation of special functions (Chapter~5) reveals surprising results:
...
\item \textbf{Efficiency advantage}: Polynomials use 4$\times$ fewer DOF (131 vs 512 at 128 intervals)
\item \textbf{Robustness}: Polynomial convergence rate consistently $\alpha \approx 4.4$; rationals vary 1.4--6.0
\end{itemize}

\textbf{Critical distinction from BVP results}: While rational collocation excels at \emph{solving} smooth BVPs (Chapters 11--15), rational \emph{interpolation} offers little advantage over piecewise polynomial splines for \emph{approximating} smooth functions. The cleared form formulation provides genuine benefits for differential operators that do not transfer to direct function representation.
