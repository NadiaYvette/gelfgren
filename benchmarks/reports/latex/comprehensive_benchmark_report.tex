...
\subsection{Critical Methodological Issue: Choice of {[}3/1] Rational}

\textbf{Important limitation:} The benchmarks used {[}3/1] rational approximants (cubic numerator, \textbf{linear denominator}). This was a \textbf{poor methodological choice} that undermines the rational method's potential.

\paragraph{Why Odd Denominator Degrees Are Problematic:}

\textbf{Complex poles come in conjugate pairs.} For oscillatory or rapidly-varying functions, poles typically occur as complex conjugates:
\begin{equation}
\text{Pole pair: } z = a \pm bi \quad \Rightarrow \quad Q(x) = {[}(x-a)^2 + b^2]
\end{equation}

\begin{itemize}
\item \textbf{Even denominator degrees} (2, 4, 6): Can represent conjugate pole pairs
  \begin{itemize}
  \item Quadratic ($m=2$): One conjugate pair $(x-a)^2 + b^2$
  \item Quartic ($m=4$): Two conjugate pairs
  \item Sextic ($m=6$): Three conjugate pairs
  \end{itemize}

\item \textbf{Odd denominator degrees} (1, 3, 5): Can only represent real poles
  \begin{itemize}
  \item Linear ($m=1$): Single real pole $(x-a)$
  \item Cubic ($m=3$): One real + one conjugate pair (suboptimal)
  \item \textbf{Requires advance knowledge} of real pole locations
  \end{itemize}
\end{itemize}

\paragraph{Implications for {[}3/1] Benchmarks:}

The {[}3/1] rational approximants used in this study:
\begin{enumerate}
\item \textbf{Cannot represent complex poles} (limited to single real pole)
\item \textbf{Inappropriate for oscillatory functions} (sine, cosine, Bessel, Mathieu)
\item \textbf{Wastes flexibility} on functions without real poles (exponential, error function)
\item \textbf{Would only be justified} if target function has known real pole (e.g., $1/(1+x)$, $\tan(x)$)
\end{enumerate}

\textbf{Consequence:} The rational approximants were handicapped by poor degree choice, potentially explaining some failures (Mathieu stagnation, Bessel struggles).

\paragraph{What Should Have Been Used:}

For fair, methodologically sound comparison:

\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Rational Type} & \textbf{Num Deg} & \textbf{Den Deg} & \textbf{Pole Capability} \\
\midrule
\textbf{[3/2]} & 3 & 2 & 1 conjugate pair \\
\textbf{[5/2]} & 5 & 2 & 1 conjugate pair (higher accuracy) \\
\textbf{[3/4]} & 3 & 4 & 2 conjugate pairs \\
\textbf{[5/4]} & 5 & 4 & 2 conjugate pairs (higher accuracy) \\
\midrule
{[}3/1] (used) & 3 & 1 & 1 real pole only \textcolor{red}{(poor choice)} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
\item \textbf{Existing {[}3/1] results are methodologically flawed}
  \begin{itemize}
  \item Rational method tested with inappropriate degree type
  \item Polynomial victory on 10/16 functions may understate rational potential (or may not!)
  \end{itemize}

\item \textbf{Proper comparison would use {[}3/2], {[}5/2], {[}3/4]}
  \begin{itemize}
  \item Even denominators enable complex pole representation
  \item Could potentially improve on oscillatory functions
  \item But: implementation attempts failed (Section~\ref{sec:dof_fairness})
  \end{itemize}

\item \textbf{Quintic Hermite results remain valid and compelling}
  \begin{itemize}
  \item Achieves machine precision (10$^{-11}$ to 10$^{-12}$)
  \item Simple, robust implementation
  \item Sets high bar for rational methods to beat
  \end{itemize}
\end{enumerate}

\textbf{Recommendation:} Future work should re-run benchmarks with {[}3/2], {[}5/2], {[}3/4] rational approximants. However, given quintic Hermite's machine precision and implementation simplicity, the practical conclusion (use polynomial methods) likely remains unchanged.

\subsection{Rational Hermite Interpolation Scheme (As Implemented)}

\textbf{Note:} The following describes the {[}3/1] scheme actually used, despite its methodological limitations.

The piecewise rational approximants used in this study are constructed via a \textbf{two-point Padé-Hermite interpolation} scheme on each subinterval $[a_i, a_{i+1}]$.

\paragraph{Construction on Each Interval:}

For a target function $f(x)$ on interval $[a, b]$, construct rational approximant:
\begin{equation}
r(x) = \frac{P(x)}{Q(x)} = \frac{p_0 + p_1 x + p_2 x^2 + p_3 x^3}{q_0 + q_1 x}
\end{equation}

\paragraph{Interpolation Conditions:}

Match function and first derivative at \textbf{both endpoints}:
\begin{align}
r(a) &= f(a), \quad r'(a) = f'(a) \\
r(b) &= f(b), \quad r'(b) = f'(b)
\end{align}

This gives \textbf{4 conditions} for the {[}3/1] rational approximant.

\paragraph{Degrees of Freedom:}

\begin{itemize}
\item Cubic numerator $P(x)$: 4 coefficients $(p_0, p_1, p_2, p_3)$
\item Linear denominator $Q(x)$: 2 coefficients $(q_0, q_1)$
\item \textbf{Normalization}: Set $q_0 = 1$ (prevents trivial solution $P = Q = 0$)
\item \textbf{Effective unknowns}: 4 + 1 = 5 per interval
\end{itemize}

\paragraph{System to Solve:}

After normalization $Q(x) = 1 + q_1 x$, the conditions become:
\begin{align}
P(a) &= f(a) Q(a) \\
P(b) &= f(b) Q(b) \\
P'(a) &= f'(a) Q(a) + f(a) Q'(a) \\
...
This is a \textbf{linear system} in the 5 unknowns $(p_0, p_1, p_2, p_3, q_1)$:
\begin{equation}
\begin{bmatrix}
1 & a & a^2 & a^3 & -f(a) a \\
1 & b & b^2 & b^3 & -f(b) b \\
0 & 1 & 2a & 3a^2 & -f'(a) a - f(a) \\
0 & 1 & 2b & 3b^2 & -f'(b) b - f(b)
\end{bmatrix}
\begin{bmatrix}
p_0 \\ p_1 \\ p_2 \\ p_3 \\ q_1
\end{bmatrix}
...
\textbf{Note:} This is an underdetermined system (4 equations, 5 unknowns), typically solved via least squares or by adding a constraint (e.g., minimize $|q_1|$ to keep denominator close to 1).

\paragraph{Derivative Computation:}

First derivatives $f'(x)$ computed via centered finite differences:
\begin{equation}
f'(x) \approx \frac{f(x + h) - f(x - h)}{2h}, \quad h = 10^{-7}
\end{equation}

\paragraph{Piecewise Assembly:}

\begin{enumerate}
\item Partition domain $[a, b]$ into $n$ subintervals: $a = x_0 < x_1 < \cdots < x_n = b$
\item On each $[x_i, x_{i+1}]$, construct {[}3/1] rational via above scheme
\item \textbf{No continuity enforced} between intervals (piecewise independent construction)
\item Evaluate piecewise function by determining which interval contains evaluation point
\end{enumerate}

\paragraph{Key Properties:}

\begin{itemize}
\item \textbf{Local construction}: Each interval independent (no global coupling)
\item \textbf{Hermite interpolation}: Matches $f$ and $f'$ at endpoints (similar to cubic Hermite)
\item \textbf{Potential discontinuity}: $r(x_{i}^+) \neq r(x_{i}^-)$ at interior knots (no $C^0$ continuity enforced)
\item \textbf{Pole risk}: No explicit pole prevention; denominator can vanish if system ill-conditioned
\item \textbf{DOF per interval}: 5 (vs cubic Hermite's 4 with shared knots)
\end{itemize}

\paragraph{Comparison to Polynomial Hermite:}

\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Feature} & \textbf{Cubic Hermite} & \textbf{[3/1] Rational} \\
\midrule
Basis & Polynomial & Rational \\
Interpolation data & $f, f'$ at endpoints & $f, f'$ at endpoints \\
Unknowns per interval & 4 coefficients & 5 coefficients \\
Continuity & $C^1$ (enforced) & $C^{-1}$ (none) \\
Construction & Linear system & Linear system \\
Pole pathology & None & Possible \\
Global DOF & $2n + 2$ & $5n$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Why This Scheme Was Chosen:}

\begin{enumerate}
\item \textbf{Simplicity}: Linear system (vs nonlinear for higher-order matching)
\item \textbf{Padé tradition}: Two-point Padé approximation is classical
\item \textbf{Data availability}: Only requires $f$ and $f'$ (easily computed)
\item \textbf{Implementation}: Available in gelfgren library as \texttt{PiecewiseRational.from\_function}
\end{enumerate}

\paragraph{Limitations of This Scheme:}

\begin{enumerate}
\item \textbf{No continuity}: Piecewise discontinuities waste approximation power
\item \textbf{Denominator degree limitation}: Linear denominator (deg 1) can only represent real poles, not complex conjugate pairs
\item \textbf{No pole control}: System doesn't explicitly prevent denominator zeros
\item \textbf{Overdetermined system resolution}: Least-squares solution may not be unique/optimal
\end{enumerate}

\textbf{For fair comparison}, one would use:
\begin{itemize}
\item {[}3/2] rational (quadratic denominator) matching $f, f', f''$ at endpoints
\item Equivalent to quintic Hermite (6 interpolation conditions)
\item Requires solving nonlinear system (see Section~\ref{sec:dof_fairness} for attempted implementation)
\end{itemize}

\section{Critical Analysis: DOF Fairness}
\label{sec:dof_fairness}
...
\subsection{Critical Methodological Flaw: {[}3/1] Degree Choice}

\textbf{The {[}3/1] rational degree type was fundamentally inappropriate:}

\paragraph{Why {[}3/1] Handicapped Rational Methods:}
\begin{itemize}
\item \textbf{Linear denominator} (deg = 1) can only represent \textbf{real poles}
\item \textbf{Complex poles come in conjugate pairs}, requiring \textbf{even denominator degree}
\item Quadratic denominator ($m=2$): $(x-a)^2 + b^2$ captures one conjugate pair
\item Linear denominator ($m=1$): $(x-a)$ captures only one real pole
\end{itemize}

\paragraph{Impact on Test Functions:}
\begin{itemize}
\item \textbf{Oscillatory functions} (sine, cosine, Bessel, Mathieu): Need complex pole representation
\item \textbf{Smooth functions} (exponential, erf): No poles, so linear denominator wastes flexibility
\item \textbf{Only appropriate for}: Functions with known real poles (e.g., logarithm near branch point)
\end{itemize}

\textbf{Rational failures may partly result from inappropriate degree choice}, not fundamental method limitations.
...
\begin{tabular}{lcc}
\toprule
\textbf{Rational Type} & \textbf{Pole Capability} & \textbf{Appropriate For} \\
\midrule
\textbf{[3/2]} & 1 conjugate pair & Oscillatory functions \\
\textbf{[5/2]} & 1 conjugate pair (higher num) & High-accuracy oscillatory \\
\textbf{[3/4]} & 2 conjugate pairs & Complex oscillations \\
{[}3/1] (used) & 1 real pole \textcolor{red}{only} & \textcolor{red}{Limited cases} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Conclusion:} Existing results are methodologically compromised. Proper comparison would use {[}3/2], {[}5/2], {[}3/4]. However, implementation attempts failed (nonlinear system convergence issues), and quintic Hermite already achieves machine precision, making the practical recommendation unchanged.
...
\textbf{If using rationals, require even denominator degrees:}
\begin{itemize}
\item \textbf{Use {[}3/2], {[}5/2], {[}3/4], {[}5/4]} (even denominators: 2, 4, 6)
\item \textbf{Never use {[}n/1], {[}n/3], {[}n/5]} (odd denominators) without advance knowledge of real poles
\item Complex poles come in conjugate pairs → require even degree
\item The {[}3/1] benchmarks in this chapter are methodologically flawed due to odd denominator
\end{itemize}
...
\item \textbf{And only with proper even denominator degrees}
\end{itemize}

\textbf{Caveat on existing {[}3/1] results:}
\begin{itemize}
\item Linear denominator was inappropriate choice
\item Rational failures (Mathieu, Bessel) may partly result from poor degree selection
\item Proper {[}3/2], {[}5/2], {[}3/4] comparison needed but implementation difficult
\item However, quintic Hermite's machine precision sets high bar
\end{itemize}
