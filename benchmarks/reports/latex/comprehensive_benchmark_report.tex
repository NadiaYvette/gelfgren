...
\section{Status: Benchmarks Not Yet Executed}

\textbf{Current Status:} The special function approximation benchmarks exist in the codebase but have not yet been executed and documented in this report. The focus of this study was on the more complex problem of \textbf{BVP solving} (Chapters 11--15), which revealed critical limitations of rational methods on non-smooth problems.

\subsection{Why Function Approximation Was Not Prioritized}

\begin{enumerate}
\item \textbf{Well-established results}: Padé approximants and piecewise rational approximation of smooth functions is well-studied with known advantages
\item \textbf{BVP solving more critical}: The question of whether rational collocation methods work for differential equations is less established and revealed unexpected catastrophic failures
\item \textbf{Resource constraints}: Comprehensive BVP benchmarks (20 problems, 6 methods, convergence analysis, report generation) consumed available effort
\end{enumerate}

\section{Expected Results (Based on Theory)}

If these benchmarks were executed, we would expect:

\begin{table}[h]
\centering
\caption{Predicted special function approximation results (expanded test suite)}
\begin{tabular}{lcc}
\hline
\textbf{Function} & \textbf{Rational Advantage?} & \textbf{Reason} \\
\hline
\multicolumn{3}{c}{\textit{Basic Functions}} \\
$e^x$ & Yes (significant) & Padé optimal for exponentials \\
$\sin(x), \cos(x)$ & Moderate & Both methods work well \\
$\tan(x)$ & Yes (near pole) & Rational handles singularity \\
$\text{erf}(x)$ & Slight & Smooth, both work \\
$\log(1+x)$ & Moderate & Rational natural form \\
Runge's function & Yes (dramatic) & Already rational \\
\hline
\multicolumn{3}{c}{\textit{Oscillatory Functions}} \\
$J_0(x)$ & Moderate & Decreasing amplitude \\
$J_n(x)$ (high $n$) & Slight to moderate & Order-dependent onset \\
$\text{ce}_n(x, q)$, $\text{se}_n(x, q)$ & Moderate & Modulated oscillations \\
$\text{Ai}(x)$ ($x < 0$) & Moderate & Oscillatory regime \\
\hline
\multicolumn{3}{c}{\textit{Elliptic \& Exponential-like}} \\
$\text{sn}(x, k)$, $\text{cn}(x, k)$ (moderate $k$) & Moderate & Trig-like for $k \sim 0.5$ \\
$\text{sn}(x, k)$, $\text{cn}(x, k)$ ($k \to 1$) & Yes (significant) & Near-hyperbolic \\
$\text{sl}(x)$, $\text{cl}(x)$ & Moderate & $k = 1/\sqrt{2}$ intermediate \\
$\text{Ai}(x)$ ($x > 0$) & Yes (significant) & Exponential decay \\
$\text{Bi}(x)$ ($x > 0$) & Yes (significant) & Exponential growth \\
\hline
\end{tabular}
\end{table}

\textbf{Key observations from expanded suite}:
\begin{itemize}
\item \textbf{Exponential-like behavior}: Rationals consistently excel ($e^x$, Airy/Bi for $x > 0$, Jacobi elliptics with $k \to 1$)
\item \textbf{Pure oscillations}: Rationals competitive but not dramatically better ($\sin$, $J_n$, Mathieu)
\item \textbf{Mixed exponential-oscillatory}: Rationals advantageous (Airy transition, $\tan$ near pole)
\item \textbf{Parameter dependence}: Results for Mathieu ($q$) and Jacobi elliptics ($k$) probe behavior \emph{at those parameters}, revealing how rationals handle that specific oscillatory character
\end{itemize}

\textbf{Key prediction}: Unlike BVP solving (where rationals catastrophically fail on boundary layers and discontinuities), function approximation of \textbf{smooth} functions should show consistent rational advantages. The critical difference is that these special functions are \textbf{known, smooth, and can be sampled at any point}—none of the pathologies (sharp gradients, discontinuities) that plague BVP rational collocation.

\section{Relationship to Extended BVP Benchmarks}

The extended BVP benchmarks (Chapters 11--15) used some functions similar to special functions as \textbf{exact solutions}:

\begin{itemize}
\item Helmholtz exponential-oscillatory: $u(x) = e^{-2x}\sin(2\pi x)$ (rationals excelled)
\item Reaction-diffusion exponential: $u(x) = \sinh(\sqrt{c}x)/\sinh(\sqrt{c})$ (rationals excelled)
\item Advection-diffusion boundary layer: $u(x) = e^{-x/\varepsilon}$ (rationals failed)
\end{itemize}

These results are \textbf{consistent} with function approximation theory: smooth exponentials are well-approximated by rationals, but sharp gradients (small $\varepsilon$) cause failure.

\section{Recommendation for Future Work}

To complete this chapter, the following work would be needed:

\begin{enumerate}
\item Execute \texttt{special\_function\_convergence.py} benchmarks
\item Generate convergence plots (error vs mesh refinement)
\item Document convergence rates for each method on each function
\item Compare against theoretical predictions (Padé error bounds)
\item Contrast with BVP results to highlight smooth vs non-smooth distinction
\end{enumerate}

\textbf{Estimated effort}: 1--2 days to run benchmarks, generate figures, and document results. Lower priority than BVP studies since results are more predictable.
