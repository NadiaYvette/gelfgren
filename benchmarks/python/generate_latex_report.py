...
\textbf{Note:} \pade{2}{2} is impossible with two-point Padé! The constraint
$m+n+1=2p$ (must be even) prevents $2+2+1=5$.

Tested configurations:
\begin{itemize}
\item \pade{2}{1}: $4N$ DOF ($\approx 4\times$ more than cubic splines)
\item \pade{3}{2}: $6N$ DOF (comparable to quintic splines per interval)
\end{itemize}
...
\item[DOF] Degrees of freedom:
  ...
  \item Rational ([2/1] Padé): $4N$ (with normalization)
  \item Rational ([3/2] Padé): $6N$
  \item Rationals use $\approx 4\times$ ([2/1]) or $6\times$ ([3/2]) more DOF per interval
  \item Note: [2/2] is impossible (constraint: $m+n+1$ must be even)
  \end{itemize}
...
\item \textbf{Key insight}: Since [2/1] rationals use $4\times$ more DOF per interval,
...
\item Efficiency: [2/1] rationals require $\approx 4\times$ more DOF
\end{itemize}
